...
def read(context: bpy.types.Context,
	...
	if fileVersion < 2:
		...
		if tmpAddr != 0:
			...
			while not finished:
				...
				elif cnkType == enums.Chunktypes.Animation and DO:
					print("Animation metadata found")
				elif cnkType == enums.Chunktypes.Morph and DO:
	...
	if file_format == 'SA1':
		...
		context.scene.saSettings.texListPointer \
			= hex8(fileR.rUInt(tmpAddr + 24))
	...
	if file_format == 'SA1':
		...
		for i in range(colCount):
			if COLs[i].model.meshPtr > 0 \
					and COLs[i].model.meshPtr not in meshes:
				meshes[COLs[i].model.meshPtr] \
					= format_BASIC.Attach.read(fileR,
											   COLs[i].model.meshPtr,
											   i,
											   labels)
		...
		for i in range(vColCount):
			...
			if ptr > 0 and ptr not in vmeshes:
				if file_format == 'SA2':
					vmeshes[ptr] \
						= format_CHUNK.Attach.read(fileR, ptr, i, labels)
				else:
		...
		if file_format == 'SA2':
			processedAttaches \
				= format_CHUNK.OrderChunks([c.model for c in COLs], vmeshes)
			format_CHUNK.ProcessChunkData([c.model for c in COLs],
										  processedAttaches,
										  noDoubleVerts,
										  None)
		else:  # sa2b
			format_GC.process_GC([c.model for c in COLs], vmeshes)
		format_BASIC.process_BASIC([c.model for c in COLs],
								   cmeshes,
								   collision=True)
	...
	bpy.context.scene.saSettings.sceneIsLevel = True
	return {'FINISHED'}
...
def write(context,
	...
	if export_format == 'SA1':
		# the sa1 format doesnt need to
		# seperate between collision and visual meshes
		...
		# bscMaterials = format_BASIC.Material.writeMaterials(fileW,
		#                                                     materials,
		#                                                     labels)
		...
		# This is always the same address btw
		...
		for m in cMeshes:
			matPtr, bscMaterials \
				= format_BASIC.Material.writeMaterials(
					fileW,
					m.materials,
					m.name,
					labels)

			mesh = format_BASIC.Attach.fromMesh(
				m,
				global_matrix,
				matPtr,
				bscMaterials,
	...
	if export_format == 'SA1':
		...
		ltFlags = 8
		# landtable flags - 8 determines that PVM/GVM's should be used
		fileW.wUInt(ltFlags)
		fileW.wFloat(drawDist)  # Draw Distance
		fileW.wUInt(COLPtr)  # geometry address
		fileW.wUInt(animPtr)  # animation address (unused rn)
		# texture file name address
		fileW.wUInt(texFileNameAddr)
		# texture list pointer (usually handled by mod)
		fileW.wUInt(texListPointer)
		...
		fileW.wUInt(COLPtr)  # geometry address
		fileW.wUInt(animPtr)  # animation address (unused rn)
		# (texFileNameAddr) texture file name address
		fileW.wUInt(texFileNameAddr)
		# texture list pointer (usually handled by mod)
		fileW.wUInt(texListPointer)
