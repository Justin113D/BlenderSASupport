...
class PolyChunk_BlendAlpha(PolyChunk_Bit):
	...
	def __init__(self, instr: enums.SA2AlphaInstructions):
		super(PolyChunk_BlendAlpha, self).__init__(
			enums.ChunkType.Bits_BlendAlpha)
...
class PolyChunk_MipmapDAdjust(PolyChunk_Bit):
	...
	def __init__(self, instr: enums.MipMapDistanceAdjust):
		super(PolyChunk_MipmapDAdjust, self).__init__(
			enums.ChunkType.Bits_MipmapDAdjust)
		self.value = instr
...
class PolyChunk_SpecularExponent(PolyChunk_Bit):
	...
	def __init__(self, exponent: float):
		super(PolyChunk_SpecularExponent, self).__init__(
			enums.ChunkType.Bits_SpecularExponent)
...
class PolyChunk_CachePolygonList(PolyChunk_Bit):
	...
	def __init__(self, index: int):
		super(PolyChunk_CachePolygonList, self).__init__(
			enums.ChunkType.Bits_CachePolygonList)
		self.index = index
...
class PolyChunk_DrawpolygonList(PolyChunk_Bit):
	...
	def __init__(self, index: int):
		super(PolyChunk_DrawpolygonList, self).__init__(
			enums.ChunkType.Bits_DrawPolygonList)
		self.index = index
...
class PolyChunk_Material(PolyChunk):
	...
	def __init__(self,
				 ...
				 specular: ColorARGB,
				 specularity: int):
		super(PolyChunk_Material, self).__init__(
			enums.ChunkType.Material_DiffuseAmbientSpecular if writeSpecular
			else enums.ChunkType.Material_DiffuseAmbient)
	...
	def read(cls,
		...
		return PolyChunk_Material(alphaInstruction,
								  diffuse,
								  ambient,
								  specular,
								  specularity), address
	...
	def write(self, fileW: fileHelper.FileWriter):
		...
		# size (amount of 2 byte sets)
		fileW.wUShort(6 if writeSpecular else 4)
...
class PolyChunk_Strip(PolyChunk):
	...
	def __init__(self,
		...
		if hasUV:
			super(PolyChunk_Strip, self).__init__(
				enums.ChunkType.Strip_StripUVN)
		else:
	...
	def getSize(self):
		size = 1
		stripSize = 3 if self.chunkType == enums.ChunkType.Strip_StripUVN \
			else 1
...
class Attach:
		...
		for v in vertexChunks:
			if v.vertexSize() * len(v.vertices) < 0xFFFF:
				...
				while len(vertices) > 0:
					vCount = min(0xFFFF - offset, math.floor(min(len(vertices)
								 * v.vertexSize(), 0xFFFF) / v.vertexSize()))

					chunkV = vertices[:vCount]
					vertices = vertices[vCount:]
					print("set:", len(chunkV), len(vertices))

					self.vertexChunks.append(
						VertexChunk(v.chunkType,
									v.weightType,
									v.weightContinue,
									offset,
									chunkV))
		...
		for p in polyChunks:
			if not isinstance(p, PolyChunk_Strip):
				...
				if p.getSize() > 0xFFFF or len(p.strips) > 0x3FFF:
					...
					polyCSize = 3 \
						if p.chunkType == enums.ChunkType.Strip_StripUVN \
						else 1
					...
					for c in range(len(strips)):
						...
						if cSize + stripSize > 0xFFFF or len(strips) >= 0x3FFF:
							self.polyChunks.append(
								PolyChunk_Strip(polyCSize == 3,
												p.flags,
												strips,
												revStrips))
							strips = list()
	...
	def getPolygons(cls, mesh: bpy.types.Mesh,
		...
		# [material specific][strip][polygon]
		strips: List[List[List[PolyVert]]] = list()
		...
		for l in polygons:
			if len(l) == 0:
				...
				continue
			stripIndices = strippifier.Strippify(l,
												 doSwaps=False,
												 concat=False,
												 name=mesh.name)
		...
		for mID, strip in enumerate(zip(strips, stripRev)):
			...
			if material is None:
				polyChunks.append(
					PolyChunk_Material(enums.SA2AlphaInstructions.SA_SRC
				...
				polyChunks.append(
					PolyChunk_Material(
						alphaflags,
						common.GetColor(saShader, 0, matProps.b_Diffuse),
						common.GetColor(saShader, 1, matProps.b_Specular),
						common.GetColor(saShader, 2, matProps.b_Ambient),
						round(matProps.b_Exponent * 255)))
				polyChunks.append(
					PolyChunk_Texture(
						common.FindTexture(saImage, matProps.b_TextureID),
						textureFlags,
	...
	def fromMesh(cls, mesh: bpy.types.Mesh,
		...
		if vertexType == 'VC':
			...
			for l in mesh.loops:
				...
				# only create the vertex if there isnt
				# one with the same color already
				foundV: Vertex = None
				...
				if foundV is None:
					foundV = Vertex(
						l.vertex_index,
						0,
						Vector3(export_matrix @ vert.co),
						Vector3(),
						col,
						0)
					verts[l.vertex_index].append(foundV)

				uv = UV(mesh.uv_layers[0].data[l.index].uv) \
					if writeUVs else UV()
			...
			for v in mesh.vertices:
				vertices.append(
					Vertex(
						v.index,
						v.index,
						Vector3(export_matrix @ v.co),
						Vector3(export_matrix @ normals[v.index]),
						None,
						0))
		...
		chunkType = enums.ChunkType.Vertex_VertexDiffuse8 \
			if vertexType == 'VC' else enums.ChunkType.Vertex_VertexNormal
		vertexChunks: List[VertexChunk] \
			= [VertexChunk(
				chunkType,
				enums.WeightStatus.Start,
				False,
				extraOffset,
				vertices)]
			...
			while chunkType != enums.ChunkType.End:
				...
				for i in range(vertexCount):
					...
					if chunkType == enums.ChunkType.Vertex_VertexDiffuse8 \
						or chunkType == \
							enums.ChunkType.Vertex_VertexNormalDiffuse8:
					...
					if chunkType == enums.ChunkType.Vertex_VertexNormal \
							or chunkType == \
							enums.ChunkType.Vertex_VertexNormalNinjaFlags \
							or chunkType == \
							enums.ChunkType.Vertex_VertexNormalDiffuse8:
		...
		if tmpAddr > 0:
			...
			while chunkType != enums.ChunkType.End:
				...
				if chunkType == enums.ChunkType.Bits_BlendAlpha:
					chunk = PolyChunk_BlendAlpha(
						enums.SA2AlphaInstructions.null)
					...
					chunk = PolyChunk_MipmapDAdjust(
						enums.MipMapDistanceAdjust.null)
				...
				elif chunkType.value > 15 and chunkType.value < 32:
					chunk, tmpAddr = PolyChunk_Material.read(
						fileR,
						chunkType,
						tmpAddr)
				elif chunkType.value > 63 and chunkType.value < 76:
					chunk, tmpAddr = PolyChunk_Strip.read(
						fileR,
						chunkType,
						tmpAddr)
	...
	for m in meshData:
		...
		for b in m.weightMap.keys():
			...
			boneData[index] = (
				...
				export_matrix @ (
					boneMap[b].inverted()
					@ m.model.origObject.matrix_world),
				list(),
				enums.ChunkType.Vertex_VertexNormalNinjaFlags)
		...
		# if the only bone is index -1, then
		# just write the entire mesh to the bone
...
def OrderChunks(models: List[common.Model], attaches: Dict[int, Attach]) -> Dict[int, processedAttach]:
	...
	for o in models:
		...
		# only if there are polychunks available
		# should we generate a processed attach.
		if len(polyChunks) > 0:
			if DO:
			...
			for c in polyChunks:
				# if its a strip
				if c.chunkType.value > 63 and c.chunkType.value < 76:
			...
			pAttaches[o.meshPtr] = processedAttach(len(pAttaches),
												   attach.name,
												   vertices,
												   polyChunks,
												   hasColor)
...
def ProcessChunkData(models: List[common.Model], attaches: Dict[int, processedAttach], noDoubleVerts: bool, armatureRoot: common.Model):
	...
	for o in models:
		...
		meshMaterials = list()
		filteredPolys = 0
		from .enums import StripFlags as sf
		for c in a.polyChunks:
			if c.chunkType.value > 63 and c.chunkType.value < 76:
				# if its a strip chunk

				f = c.flags
				tmpMat["b_ignoreLighting"] = bool(f & sf.IGNORE_LIGHT)
				tmpMat["b_ignoreSpecular"] = bool(f & sf.INGORE_SPECULAR)
				tmpMat["b_ignoreAmbient"] = bool(f & sf.IGNORE_AMBIENT)
				tmpMat["b_useAlpha"] = bool(f & sf.USE_ALPHA)
				tmpMat["b_doubleSided"] = bool(f & sf.DOUBLE_SIDE)
				tmpMat["b_flatShading"] = bool(f & sf.FLAT_SHADING)
				tmpMat["b_useEnv"] = bool(f & sf.ENV_MAPPING)
				tmpMat["b_unknown"] = bool(f & sf.Unknown)
			...
			# if material or blendalpha
			elif (c.chunkType.value > 15 and c.chunkType.value < 32) \
					or c.chunkType == enums.ChunkType.Bits_BlendAlpha:
				instr = c.alphaInstruction
				from .enums import SA2AlphaInstructions as ai

				if instr & ai.SA_INV_DST == ai.SA_INV_DST:
					tmpMat["b_srcAlpha"] = 'INV_DST'
				elif instr & ai.SA_DST == ai.SA_DST:
					tmpMat["b_srcAlpha"] = 'DST'
				elif instr & ai.SA_INV_SRC == ai.SA_INV_SRC:
					tmpMat["b_srcAlpha"] = 'INV_SRC'
				elif instr & ai.SA_INV_OTHER == ai.SA_INV_OTHER:
					tmpMat["b_srcAlpha"] = 'INV_OTHER'
				elif instr & ai.SA_SRC:
					tmpMat["b_srcAlpha"] = 'SRC'
				elif instr & ai.SA_OTHER:
					tmpMat["b_srcAlpha"] = 'OTHER'
				elif instr & ai.SA_ONE:
					tmpMat["b_srcAlpha"] = 'ONE'
				else:
					tmpMat["b_srcAlpha"] = 'ZERO'

				if instr & ai.DA_INV_DST == ai.DA_INV_DST:
					tmpMat["b_destAlpha"] = 'INV_DST'
				elif instr & ai.DA_DST == ai.DA_DST:
					tmpMat["b_destAlpha"] = 'DST'
				elif instr & ai.DA_INV_SRC == ai.DA_INV_SRC:
					tmpMat["b_destAlpha"] = 'INV_SRC'
				elif instr & ai.DA_INV_OTHER == ai.DA_INV_OTHER:
					tmpMat["b_destAlpha"] = 'INV_OTHER'
				elif instr & ai.DA_SRC:
					tmpMat["b_destAlpha"] = 'SRC'
				elif instr & ai.DA_OTHER:
					tmpMat["b_destAlpha"] = 'OTHER'
				elif instr & ai.DA_ONE:
				...
				if c.filtering == tf.Point:
					tmpMat["b_texFilter"] = 'POINT'
				elif c.filtering == tf.Bilinear:
					tmpMat["b_texFilter"] = 'BILINEAR'
				elif c.filtering == tf.Trilinear:
					tmpMat["b_texFilter"] = 'TRILINEAR'
				elif c.filtering == tf.Blend:
					tmpMat["b_texFilter"] = 'BLEND'

				idf = enums.TextureIDFlags

				tmpMat["b_d_025"] = bool(c.flags & idf.D_025)
				tmpMat["b_d_050"] = bool(c.flags & idf.D_050)
				tmpMat["b_d_100"] = bool(c.flags & idf.D_100)
				tmpMat["b_d_200"] = bool(c.flags & idf.D_200)
				tmpMat["b_clampU"] = bool(
					c.flags & idf.CLAMP_U)
				tmpMat["b_clampV"] = bool(
					c.flags & idf.CLAMP_V)
				tmpMat["b_mirrorU"] = bool(
					c.flags & idf.FLIP_U)
				tmpMat["b_mirrorV"] = bool(
					c.flags & idf.FLIP_V)
				...
				tmpMat["b_d_025"] = bool(
					c.value & enums.MipMapDistanceAdjust.D_025)
				tmpMat["b_d_050"] = bool(
					c.value & enums.MipMapDistanceAdjust.D_050)
				tmpMat["b_d_100"] = bool(
					c.value & enums.MipMapDistanceAdjust.D_100)
				tmpMat["b_d_200"] = bool(
					c.value & enums.MipMapDistanceAdjust.D_200)
